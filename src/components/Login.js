  ...
  if (isSignUp) {
   ...
   return (
     <div className='fixed top-0 left-0 right-0 bottom-0 z-50 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      <form onSubmit={handleLogin} className='relative bg-white p-10 rounded-xl text-slate-500 w-96 max-w-[90%]'>
        ...
        <button type="submit" className='bg-blue-600 w-full text-white py-2 rounded-full'>Login</button>

        <p className='mt-5 text-center'>Don't have an account? <span className='text-blue-600 cursor-pointer' onClick={handleSignUpToggle}>Sign up</span></p>
